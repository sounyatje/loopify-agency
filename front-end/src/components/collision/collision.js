function init() {
    ...
    const mapPandaAssets = () =>{
      return Object.keys(directions).map(dir =>{
        return  `<svg x="0px" y="0px" width="100%" height="20%" viewBox="0 0 624 48">${decode(pandaSvg[dir])}</svg>`
      }).join('')
    }
    ...
    const collisionCheck = () =>{
      ...
      // * Uncomment below to see collision
...
const updateProgress = (current, total) => {
    ...
    loadingText.textContent = `Chargement... ${progressPercentage}%`;

    // Cacher la jauge une fois le chargement terminé
    if (current === total) {
        setTimeout(() => {
            loadingScreen.style.display = 'none'; // Masquer l'écran de chargement
        }, 500); // Petite pause pour montrer le 100%
    }
};

    // Create panda and set up collision check
    new Array(pandaNumber).fill('').map(()=>{
